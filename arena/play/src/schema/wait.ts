...
import { field, isJsonString, type JsonValue, type SessionIdentity } from 'src/schema/primitives';
...
const isWakeReason = (value: JsonValue): value is WakeReason =>
  isJsonString(value) && V2_WAKE_REASONS.has(value);
...
export const decodeWait = (
  ...
  Effect.gen(function* () {
    ...
    if (field(raw, 'agent_id') !== session.agentId) {
      return yield*invalid('v2 wait response agent');
    }
    const wakeReason = field(raw, 'wake_reason');
    if (!isWakeReason(wakeReason)) {
      return yield*invalid('v2 wait wake reason');
    }
    ...
    const broken =
      ...
      (wakeReason === 'revision_changed' &&
        !(
          until === 'revision' &&
          revision !== null &&
          afterStateToken !== null &&
          revision.state_token !== afterStateToken
        ));
    if (broken) {
      return yield*invalid('v2 wait wake contract');
    }
