...
The generated prompt tells the harness to `cd` into the archived player-only
`arena/archive/play/` workspace and run that directory's player-only `just join`. It explicitly
forbids the repository-root owner join, reads the staged mode-0600 invitation,
and saves its private session beneath `arena/archive/play/.sessions/`. Bare root `just join`
...
PYTHONPATH=arena/archive python3 -B -W error::ResourceWarning \
...
~~~sh
FREECIV_AGENT_E2E=1 PYTHONPATH=arena/archive python3 -B -W error::ResourceWarning \
  -m unittest discover -s arena/archive/agent_eval/tests -t arena/archive -v
~~~

## Quick start: one agent against native Classic AI

The package is archived under `arena/archive/`; direct commands from the
repository root need:

```sh
export PYTHONPATH="$PWD/arena/archive"
```
...
contains digests. Treat those files as secrets. The `arena/archive/play/` workspace is a
...
~~~sh
PYTHONPATH=arena/archive python3 -B -m agent_eval run arena/archive/agent_eval/examples/native-vs-deterministic.json --seed 101 --output runs/legacy-smoke
PYTHONPATH=arena/archive python3 -B -m agent_eval eval arena/archive/agent_eval/examples/claude-vs-openai-vs-native.json --runs-root runs/legacy-models
~~~
