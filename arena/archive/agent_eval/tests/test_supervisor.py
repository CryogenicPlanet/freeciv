...
class SupervisorTests(unittest.TestCase):
            ...
            [
                ...
                "lua unsafe-file " + str(
                    (
                        ...
                        / "arena" / "archive" / "agent_eval" / "bridge.lua"
                    ).resolve()
                ),
            ],
        )
    ...
    def test_bridge_zero_transport_timeout_means_no_curl_deadline(self):
        source = Path("arena/archive/agent_eval/bridge.lua").read_text(encoding="utf-8")
    ...
    def test_validation_and_policy_free_service_module(self):
        ...
        source = Path("arena/archive/agent_eval/supervisor.py").read_text()
...
""",
    ...
    def test_just_surfaces_native_watch_replay_and_safe_agent_prompt(self):
        ...
        self.assertIn(
            "arena/archive/play/.invites/game_12345678901234567890.json",
            rendered_invite,
        )
        ...
        self.assertIn(
            f"cd {repo.resolve() / 'arena' / 'archive' / 'play'}",
            bare.stdout,
        )
    ...
    def _obsolete_just_replay_reuses_healthy_vite_for_picker_or_direct_game(self):
        ...
        vite_entry = repo / "arena/viewer/node_modules/vite/bin/vite.js"
    ...
    def _obsolete_just_replay_cleans_only_gateway_spawned_before_vite_conflict(self):
        repo = Path(__file__).parents[4].resolve()
