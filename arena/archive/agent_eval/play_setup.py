...
Each workspace is a copy of ``arena/archive/play/`` with the game invitation staged, a
...
# The compiled Effect/Bun client. Workspaces are provisioned with this binary
# as `./play` — the justfile is gone, and the archived Python client stays in
# the copy only as the byte-diff oracle's reference implementation.
PLAY_BINARY = REPO_ROOT / "arena" / "play" / "dist" / "play"
...
# arena/play/src/services/prog-prefix.ts.
_PROG_VERBS = (
...
def _stage_invite(
    ...
    existing = (
        repo_root / "arena" / "archive" / "play" / ".invites"
        / f"{game_id}.json"
    )
    if existing.is_file():
...
def _materialize(
    ...
    if tls_ca is not None:
        ...
        # the client reads it back per request (`arena/play/NOTES.md` §I.3.1).
