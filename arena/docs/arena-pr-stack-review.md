...
`arena/archive/agent_eval/replay_derive_cli.py` and `ReplayDerivationPython` remain an explicit bridge until native derivation lands. The `local_stack --ts-gateway` switch is expected to remain during migration and is opt-in; the default gateway is Python.
