...
const seat = (options: SeatOptions = {}): Seat => {
  ...
  if (revision !== null) {
    Effect.runSync(
      scratch.files.writeJson(path.join(home, 'codex-test.v2-state'), clientState(revision))
    );
  }
...
const golden = (
  ...
  const actual = show(fixture, overrides);
...
describe('the default listing', () => {
  test('bare show prints the header, the delta and the catalog', () => {
    golden('current/default', seat());
  });

  test('--json wraps the same lines in the show envelope', () => {
    golden('current/default_json', seat(), { json: true });
  });

  test('a mirror missing one file simply does not list it', () => {
    golden(
      'nopriced/default',
  ...
  test('an empty mirror names `just state --section overview` before `just turn`', () => {
    const fixture = seat({ files: [] });
    const { stdout, stderr } = show(fixture);
    expect(stdout).toBe('');
  ...
  test('--grep over an empty mirror earns the same refusal', () => {
    const fixture = seat({ files: [] });
    const { stderr } = show(fixture, { grep: 'anything' });
...
describe('a named section', () => {
  test('one file prints byte for byte, and nothing else', () => {
    golden('current/units', seat(), { name: 'units' });
    golden('current/map', seat(), { name: 'map' });
    golden('current/nations', seat(), { name: 'nations' });
    // `phase.json` carries no `# rev` stamp, so it is never banner-eligible.
    golden('current/phase', seat(), { name: 'phase' });
  });

  test('a file this seat has not written names `just turn`', () => {
    golden('empty/units', seat({ files: [] }), { name: 'units' });
  });

  test('an unknown name lists the file names, sorted', () => {
    golden('current/unknown_name', seat(), { name: 'bogus' });
  });

  test('an alias-shaped name that is absent names its own repair', () => {
    golden('current/alias_u9', seat(), { name: 'u9' });
  });

  test('a traversal attempt never reaches outside the mirror', () => {
    golden('current/hostile_dotdot', seat(), { name: '../codex-test.v2-state' });
    golden('current/hostile_abs', seat(), { name: '/etc/passwd' });
  });
});

describe('an entity alias', () => {
  test('u1 is its unit row followed by its option projection', () => {
    golden('current/alias_u1', seat(), { name: 'u1' });
  });

  test('c1 and r1 reach the city and relation tables the same way', () => {
    golden('current/alias_c1', seat(), { name: 'c1' });
    golden('current/alias_r1', seat(), { name: 'r1' });
  });

  test('an option projection with no row prints alone, with no leading blank', () => {
    golden('current/alias_unscoped', seat(), { name: 'unscoped' });
  });

  test('--json carries the alias as the selection', () => {
    golden('current/alias_u1_json', seat(), { name: 'u1', json: true });
  });
});

describe('--grep', () => {
  test('a literal match prints `label:line: text`', () => {
    golden('current/grep_settlers', seat(), { grep: 'Settlers' });
  });

  test('the literal search is case-insensitive', () => {
    golden('current/grep_case', seat(), { grep: 'settlers' });
  });

  test('no match says so, with the pattern in CPython `repr` form', () => {
    golden('current/grep_missing', seat(), { grep: 'zzz-not-here' });
  });

  test('literal means literal: a metacharacter stands for itself', () => {
    golden('current/grep_literal_dot', seat(), { grep: 'Settl.rs' });
  });

  test('--regex opts into the engine', () => {
    golden('current/grep_regex_dot', seat(), { grep: 'Settl.rs', regex: true });
  });

  test('a catastrophic pattern is just text without --regex, and answers at once', () => {
    const started = performance.now();
    golden('current/grep_catastrophic', seat(), { grep: '(a|aa)+$' });
    expect(performance.now() - started).toBeLessThan(1000);
  });

  test('--regex refuses a quantifier applied to an already quantified group', () => {
    golden('current/grep_regex_nested', seat(), { grep: '(a+)+$', regex: true });
  });

  test('--regex reports an uncompilable pattern and names the flagless form', () => {
    const { stdout, stderr } = show(seat(), { grep: '(unbalanced', regex: true });
    expect(stdout).toBe('');
   ...
   */
  test('a pattern too large for the engine refuses in words, not in a stack trace', () => {
    for (const grep of ['\\w'.repeat(100), '\\b'.repeat(21)]) {
      const { stdout, stderr } = show(seat(), { grep, regex: true });
      expect(stdout).toBe('');
      expect(stderr).toBe(
  ...
  test('--regex gives up when it spends its wall-clock budget', () => {
    const ticks = [0, 0, 99];
    const { stderr } = show(seat(), {
      grep: '(a|aa)+$',
      regex: true,
  ...
  test('a pattern over 200 characters is refused before anything is read', () => {
    golden('current/grep_long', seat(), { grep: 'x'.repeat(201) });
  });

  test('the match list stops at 200 and says it stopped', () => {
    const rows = Array.from({ length: 250 }, (_unused, index) => `u${index}\tSettlers`);
    const fixture = seat({
      files: [['state/units.tsv', `# rev 7 turn 3\n${rows.join('\n')}\n`]],
    });
    const { stdout } = show(fixture, { grep: 'Settlers' });
  ...
  test('exactly 200 matches is not truncated', () => {
  ...
  test('--json carries `grep PATTERN` as the selection', () => {
    golden('current/grep_settlers_json', seat(), { grep: 'Settlers', json: true });
  });
});

describe('flag precedence', () => {
  test('a name and --grep together are refused', () => {
    golden('current/name_and_grep', seat(), { name: 'units', grep: 'Settlers' });
  });

  test('--regex without --grep is refused', () => {
    golden('current/regex_without_grep', seat(), { regex: true });
  });
});

describe('the staleness banner', () => {
  test('a current projection carries no banner', () => {
    golden('current/units', seat({ revision: 7 }), { name: 'units' });
  });

  test('a lagging projection leads with the re-verification line', () => {
    golden('stale/units', seat({ revision: 12 }), { name: 'units' });
    golden('stale/map', seat({ revision: 12 }), { name: 'map' });
    golden('stale/nations', seat({ revision: 12 }), { name: 'nations' });
  });

  test('the option projection carries the banner and the file still says rev 7', () => {
    const fixture = seat({ revision: 12 });
    golden('stale/alias_u1', fixture, { name: 'u1' });
    // Written at read time: the file itself is untouched.
    const stored = Effect.runSync(
      fixture.files.readText(
  ...
  test('the default listing, the alias rows and --grep all carry it', () => {
    golden('stale/default', seat({ revision: 12 }));
    golden('stale/alias_c1', seat({ revision: 12 }), { name: 'c1' });
    golden('stale/alias_r1', seat({ revision: 12 }), { name: 'r1' });
    golden('stale/alias_unscoped', seat({ revision: 12 }), { name: 'unscoped' });
    golden('stale/grep_settlers', seat({ revision: 12 }), { grep: 'Settlers' });
    golden('stale/grep_missing', seat({ revision: 12 }), { grep: 'zzz-not-here' });
    golden('stale/alias_u1_json', seat({ revision: 12 }), { name: 'u1', json: true });
  });

  test('a file with no revision stamp is never judged stale', () => {
    golden('stale/phase', seat({ revision: 12 }), { name: 'phase' });
  });

  test('an unreadable private cache annotates nothing and blocks nothing', () => {
    golden('nostate/default', seat({ revision: null }));
    golden('nostate/units', seat({ revision: null }), { name: 'units' });
    // The same holds for a `.v2-state` that exists but does not parse.
    const broken = seat({ revision: 12 });
    Effect.runSync(
      broken.files.writeText(`${broken.sessionPath.slice(0, -5)}.v2-state`, 'not json\n')
    );
    golden('nostate/units', broken, { name: 'units' });
  });

  test('the sources of a bare listing are every present file', () => {
    const fixture = seat();
    const sources = Effect.runSync(
      provideTestLayer(showSources(fixture.sessionPath, '', ''), fixture.layer)
    );
    expect(sources).toHaveLength(MIRROR.length);
  });

  test('the sources of an alias are the three row tables plus its option file', () => {
    const fixture = seat();
    const sources = Effect.runSync(
      provideTestLayer(showSources(fixture.sessionPath, 'u1', ''), fixture.layer)
    );
...
describe('the options listing', () => {
  test('only `.txt` files count, dotfiles never do, and the order is sorted', () => {
    const fixture = seat({
      files: [
        ...MIRROR,
    ...
    const names = Effect.runSync(
      provideTestLayer(showOptionFiles(fixture.sessionPath), fixture.layer)
    );
...
describe('CPython `str.strip()`, not `String.trim()`', () => {
  test('U+0085 and U+001C-U+001F are stripped from a name', () => {
    golden('current/name_x85_lead', seat(), { name: '\x85units' });
    golden('current/name_x1c_lead', seat(), { name: '\x1cunits' });
    golden('current/name_x1f_lead', seat(), { name: '\x1funits' });
    // The characters both classes agree about still work, of course.
    golden('current/name_nbsp_lead', seat(), { name: '\xa0units' });
    golden('current/name_space_lead', seat(), { name: '  units  ' });
  });

  test('U+FEFF is not stripped, so SHOW_NAME_RE refuses the name', () => {
    golden('current/name_bom_lead', seat(), { name: '\ufeffunits' });
    // Trailing, too: `u1` with a trailing U+FEFF is not the alias `u1`.
    golden('current/name_bom_trail', seat(), { name: 'u1\ufeff' });
  });

  test('the same class decides what --grep actually searches for', () => {
    // Stripped: the needle is `Settlers`, and the row is found.
    golden('current/grep_x85_settlers', seat(), { grep: '\x85Settlers' });
    // Not stripped: the needle keeps the BOM and matches nothing.
    golden('current/grep_bom_settlers', seat(), { grep: '\ufeffSettlers' });
  });

  test('a pattern of nothing but whitespace strips to empty and is no --grep at all', () => {
    golden('current/grep_x85_only', seat(), { grep: '\x85' });
    golden('current/grep_nbsp_only', seat(), { grep: '\xa0' });
    // …which is what makes `--regex` with it a refusal rather than a search.
    golden('current/grep_x85_only_regex', seat(), { grep: '\x85', regex: true });
    // A BOM survives the strip, so it *is* a pattern — and it matches nothing.
    golden('current/grep_bom_only', seat(), { grep: '\ufeff' });
  });
   ...
   */
  test('the alias column is compared after CPython’s strip', () => {
    const rows =
      '# rev 7 turn 3\n# units 3/3 complete\nalias\tunit\n' +
      '\x1cu1\x85\tSettlers\n' +
      'u2\ufeff\tWorkers\n' +
    ...
    expect(show(seat({ files }), { name: 'u1' }).stdout).toBe(`units: u1\n\n${options}`);
    expect(show(seat({ files }), { name: 'u2' }).stderr).toBe(
    ...
    expect(show(seat({ files }), { name: 'u3' }).stdout).toBe('units:   u3  \tExplorer\n');
  });

  test('a name of nothing but whitespace is no name, so it is not "both"', () => {
    golden('current/grep_bom_and_name', seat(), { name: '\ufeff', grep: 'Settlers' });
  });

  test('an unprintable needle is echoed through CPython `repr`', () => {
    golden('current/grep_interior_nonprintable', seat(), { grep: 'Set\u0378tlers' });
    golden('current/grep_nbsp_interior', seat(), { grep: 'Set\xa0tlers' });
    golden('current/grep_astral_nonprintable', seat(), { grep: 'a\u{e0001}b' });
  });
});
