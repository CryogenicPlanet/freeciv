...
const sources = withTestFileSystem((files) =>
  Effect.gen(function* () {
    const entries = yield* files.readDirectory(SRC, { recursive: true });
    const sourceFiles = entries.filter((entry) => entry.endsWith('.ts'));
    return yield* Effect.forEach(sourceFiles, (file) =>
      Effect.map(files.readFileString(path.join(SRC, file)), (text) => [file, text] as const)
    );
  })
).pipe(Effect.orDie);
...
/** Remaining `let` debt outside the parsers, pinned. */
const LET_BUDGET = new Map<string, number>([
...
describe('the imperative ratchet', () => {
  effectTest('no literal try/catch anywhere in src', () =>
    Effect.gen(function* () {
      for (const [file, text] of yield* sources) {
        expect(count(text, /(?<![A-Za-z])try\s*\{/g), file).toBe(0);
      }
    })
  );

  effectTest('infinite loops live only in the CPython scanner ports', () =>
    Effect.gen(function* () {
      for (const [file, text] of yield* sources) {
        const loops = count(text, /for \(;;\)|while \(true\)/g);
        if (!PARSER_FILES.has(file)) expect(loops, file).toBe(0);
      }
    })
  );

  effectTest('let budgets only go down', () =>
    Effect.gen(function* () {
      for (const [file, text] of yield* sources) {
        const lets = count(text, /(^|[^A-Za-z])let /gm);
        const budget = LET_BUDGET.get(file) ?? 0;
  ...
  effectTest('no module-level mutable bindings', () =>
    Effect.gen(function* () {
      for (const [file, text] of yield* sources) {
