...
import { isActorType } from 'src/schema/ids';
...
export const scalar = (value: JsonValue): string => {
  ...
  if (isJsonNumber(value)) return Number.isInteger(value) ? String(value) : formatG(value);
...
export const plainName = (value: JsonValue): string | null => {
  ...
  if (isJsonString(name) && name !== '') return name;
  const identifier = field(value, 'id');
  if (isJsonString(identifier) && identifier !== '') return identifier;
  return null;
};
...
export const requestedScope = (actorId: string, targetId: string): PageScope | null => {
  ...
  const actorType = actorId.split('_', 1)[0] ?? '';
  if (!isActorType(actorType)) return null;
...
export const probabilityText = (probability: JsonValue): string => {
  ...
  const kind = field(probability, 'kind');
  const low = field(probability, 'minimum_percent');
  const high = field(probability, 'maximum_percent');
  if (!isJsonString(kind) || kind === '' || !isJsonNumber(low) || !isJsonNumber(high)) {
...
export const schemaSummary = (schema: JsonValue): Effect.Effect<string, PlayerError> => {
  ...
  const requiredNames = new Set(
    Array.isArray(required) ? required.filter((name): name is string => isJsonString(name)) : []
  );
  const parts = Object.entries(properties).map(([name, specification]) => {
    const kind = ((): string => {
      ...
      if (isJsonString(declared) && declared !== '') return declared;
...
export const rowAlias = (
  ...
  const identifier = isJsonObject(item) ? field(item, key) : null;
  if (!isJsonString(identifier) || identifier === '') {
...
export const signed = (value: JsonValue): string => {
  if (!isWholeNumber(value)) return scalar(value);
